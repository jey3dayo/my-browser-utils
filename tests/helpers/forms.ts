...
export async function selectBaseUiOption(window: Window, trigger: HTMLElement, optionText: string): Promise<void> {
  trigger.click();
  await flush(window);
  ...
  const option = Array.from(popup.querySelectorAll<HTMLElement>('.mbu-select-item')).find(item =>

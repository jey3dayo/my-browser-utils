...
describe('ToastHost', () => {
  it('renders toasts in the document surface', async () => {
    ...
    await act(async () => {
      root.render(<ToastHost portalContainer={document.body} toastManager={toastManager} />);
      await flushEffects();
    });
    ...
    expect(document.body.querySelector('.mbu-toast-root')?.textContent).toContain('完了しました');
  ...
  it('renders portalled toasts inside the shadow root', async () => {
    ...
    const root = createRoot(container);
    await act(async () => {
      root.render(<ToastHost portalContainer={shadow} toastManager={toastManager} />);
      await flushEffects();
    });
    ...
    expect(shadow.querySelector('.mbu-toast-root')?.textContent).toContain('失敗しました');

...
describe('content overlay (React + Shadow DOM)', () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    (globalThis as unknown as { __MBU_CONTENT_STATE__?: unknown }).__MBU_CONTENT_STATE__ = undefined;
  ...
  it('shows an error toast in the shadow root when clipboard write fails', async () => {
    ...
    const copyButton = shadow?.querySelector<HTMLButtonElement>('[data-testid="overlay-copy"]');
  ...
  it('renders event overlay with table + quote, hides link-copy, and keeps pin next to close', async () => {
    ...
    expect(shadow?.textContent).toContain('宮城・セキスイハイムスーパーアリーナ');
    ...
    const hasLinkCopyButton = Array.from(shadow?.querySelectorAll('button') ?? []).some(btn =>
      btn.textContent?.includes('リンクコピー'),
    );

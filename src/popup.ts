...
async function initializePopup(): Promise<void> {
  const settings = await chrome.storage.sync.get(['autoEnableSort']) as StorageData;
...
async function loadPatterns(): Promise<void> {
  const { domainPatterns = [] } = await chrome.storage.sync.get(['domainPatterns']) as StorageData;
